    ...
    User.findByIdAndUpdate(req.params.id, {
        ...
        phNumber: req.body.phNumber
            ...
            if (err) {
                ...
                console.log(`E11000 duplicate key error collection: test-a.users index: phNumber_1 dup key: { username: "${req.body.phNumber}" }`);
                if (err.message === 'E11000 duplicate key error collection: test-a.users index: username_1 dup key: { username: ' + '"' + req.body.username + '" }') {
                    req.flash("error", "A User with the given Username exists!");
                    return res.redirect("back"); 
                }
                if (err.message === 'E11000 duplicate key error collection: test-a.users index: email_1 dup key: { email: ' + '"' + req.body.email + '" }') {
